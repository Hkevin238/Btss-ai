...
# 2. CSS Y'ISURA N'IMITATSI (UI Styling)
bg_css = """
<style>
...
<div class="top-bar">
    ...
    <div style="font-size: 1.2rem; color: #cccccc;">⋮</div>
</div>
""",
...
if not groq_api_key:
    st.error("⚠️ GROQ_API_KEY ntiyabonywe muri Streamlit Secrets cyangwa Environment!")
    st.stop()

# Gufungura OpenAI client yerekera kuri Groq
client = OpenAI(
...
# 4. SYSTEM PROMPT
SYSTEM_PROMPT = (
    "You are the official AI assistant for Bulinga TSS. Created by Developer Kevin.\n"
...
# Kwerekana ubutumwa burimo
for message in st.session_state.messages:
    if message["role"] != "system":
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

# 5. GUSUBIZA UBUTUMWA BW'UMUKORESHA
